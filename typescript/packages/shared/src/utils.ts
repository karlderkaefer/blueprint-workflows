 ...
 */
export async function detectChangedHelmCharts(
  pathGitRepository: path.FormatInputPathObject,
  helmChartListingFileContent: string,
  branchName?: string,
  baseBranchName: string = 'main',
  sourceGitRepoUrl?: string,
  targetGitRepoUrl?: string,
  token?: string
): Promise<Set<string>> {
  ...
  try {
    ...
    const changedFiles: string[] = result.stdout.split('\n').filter(f => f.trim() !== '')
